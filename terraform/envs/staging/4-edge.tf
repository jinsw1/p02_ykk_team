...
############################################
module "project02_staging_alb" {
  source = "../../modules/alb"
  name   = "project02-staging-alb"
  vpc_id = local.vpc_id

  subnets = [
    module.project02_staging_public_subnet_alb_a.subnet_id,
    module.project02_staging_public_subnet_alb_b.subnet_id
  ]
...
resource "aws_lb_target_group_attachment" "staging_was1" {
  target_group_arn = module.project02_staging_alb.tg_arn
  target_id        = module.project02_staging_was01_ec2.instance_id
  port             = 80
}

resource "aws_lb_target_group_attachment" "staging_was2" {
  target_group_arn = module.project02_staging_alb.tg_arn
  target_id        = module.project02_staging_was02_ec2.instance_id
  port             = 80
}
...
resource "aws_lb_listener" "http" {
  load_balancer_arn = module.project02_staging_alb.alb_arn
  port              = 80
  protocol          = "HTTP"
...
module "project02_dns" {
  ...
  name    = "staging"
  type    = "CNAME"
  content = module.project02_staging_alb.dns_name
  proxied = true
}